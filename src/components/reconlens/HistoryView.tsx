...
import { ArrowUpRight, CheckCircle2, Filter, GitCompare, Search, Trash2 } from "lucide-react";
...
export function HistoryView({ onRescan }: { onRescan: (url: string) => void }) {
    ...
    <div className="mx-auto w-full max-w-7xl px-6 py-10 lg:px-10">
      <header className="mb-8">
        ...
        <p className="mt-2 max-w-2xl text-[15px] text-muted-foreground">
          Review past scans, search across targets and technologies. Click a row to re-run the analysis.
        </p>
      </header>
      ...
      <Card className="p-4">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Button disabled={selected.size !== 2} className="h-10 gap-2">
            <GitCompare className="h-4 w-4" /> Compare{selected.size > 0 ? ` (${selected.size}/2)` : ""}
          </Button>
          {history.length > 0 && (
            <Button variant="ghost" className="h-10 gap-2 text-muted-foreground" onClick={handleClear}>
      ...
      {history.length === 0 ? (
        <Card className="mt-4 py-16 text-center text-sm text-muted-foreground">
          No analyses yet. Run a scan from the Analyze tab.
        </Card>
      ) : (
      ...
      <div className="mt-4 flex items-center justify-between text-[12.5px] text-muted-foreground">
        <span>Showing {filtered.length} of {history.length} analyses</span>
